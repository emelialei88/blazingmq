...
def test_remove_appid_reconfigure(cluster: Cluster):
    ...
    # ---------------------------------------------------------------------
    # Consumer connected to victim_app_id receives the message and tries to confirm.

    assert consumers[victim_app_id].wait_push_event(timeout=5)
    assert (
        consumers[victim_app_id].confirm(tc.URI_FANOUT, "*", block=True, no_except=True)
        != Client.e_SUCCESS
    )

    # ---------------------------------------------------------------------
    # Consumers connected to other app ids receive the message and tries to confirm.

    for app_id in app_ids:
        assert consumers[app_id].wait_push_event(timeout=5)
        consumers[app_id].confirm(tc.URI_FANOUT, "*", succeed=True)
